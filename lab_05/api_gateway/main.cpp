...
public:
                       ...
                       [[maybe_unused]] HTTPServerResponse &response)
    {
        ...
        std::string base_url_conf = "http://lab_05_conf:8080";
        std::string base_url_report = "http://lab_05_report:8080";
        std::string base_url_data;

        if (hasSubstr(request.getURI(),"/conf")||
            hasSubstr(request.getURI(),"/confs"))
            base_url_data = base_url_conf;
        else if (hasSubstr(request.getURI(),"/report")||
            hasSubstr(request.getURI(),"/reports"))
            base_url_data = base_url_report;
