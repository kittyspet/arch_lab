                       ...
                       HTTPServerResponse &response)
    {
        ...
        try
        {
            ...
            if (hasSubstr(request.getURI(), "/reports") && (request.getMethod() == Poco::Net::HTTPRequest::HTTP_GET))
            {
                ...
                auto results = database::Report::read_by_author_id(id);
                Poco::JSON::Array arr;
                for (auto s : results)
                    arr.add(s.toJSON());
            ...
            {
                long report_id = atol(form.get("id").c_str());

                std::optional<database::Report> result = database::Report::read_by_id(report_id);
                if (result)
                {
            ...
            else if (request.getMethod() == Poco::Net::HTTPRequest::HTTP_POST)
            {
                ...
                report.author_id() = atol(form.get("author_id").c_str());
