...
namespace database
{
    ...
    std::vector<User> User::user_log(std::string login)
    {
        try
        {
            Poco::Data::Session session = database::Database::get().create_session();
            ...
            User a;
            login += "%";
            select << "SELECT id, first_name, last_name, email, login, password FROM users where login LIKE $1",
                into(a._id),
                into(a._first_name),
                into(a._last_name),
                into(a._email),
    ...
    void User::save_to_mysql()
    {
        ...
        try
        {
            ...
            insert << "INSERT INTO users (first_name,last_name,email,login,password) VALUES($1, $2, $3, $4, $5, $6)",
                use(_first_name),
                use(_last_name),
                use(_email),
                use(_login),
